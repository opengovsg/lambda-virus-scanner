...
describe('handler', () => {
  ...
  it('should delete file and log and return 400 status with virus metadata if file is malicious', async () => {
    ...
    const deleteS3File = jest.fn().mockResolvedValueOnce('ok')
    MockS3Service.prototype.deleteS3File = deleteS3File
    ...
    expect(deleteS3File).toHaveBeenCalledWith({
  ...
  it('should move file to clean bucket and log and return 200 if file is not malicious', async () => {
    ...
    const bodyJson = JSON.parse(result.body) as {
      message: string
      cleanFileKey: string
    }
